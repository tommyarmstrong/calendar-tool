...
def wait_for_policy_availability(
    iam: Any, policy_arn: str, max_retries: int = 10
) -> None:
    """
...
def wait_for_lambda_ready_for_layers(
    lmb: Any, function_name: str, max_retries: int = 10
) -> None:
    """
...
def attach_existing_policies(iam: Any, role_name: str, policy_names: list[str]) -> None:
    ...
    for policy_name in policy_names:
        try:
            ...
            print(
                f"✅ Successfully attached policy '{policy_name}' to role '{role_name}'"
            )
        except ClientError as e:
...
def create_or_update_function(
    ...
    if not function_exists(lmb, cfg.function_name):
        ...
        lmb.update_function_code(
            FunctionName=cfg.function_name, ZipFile=code_zip, Publish=True
        )
        wait_for_lambda_ready(lmb, cfg.function_name)
        print("Code updated.")
...
def run() -> None:
    ...
    ap.add_argument(
        "--policy-file",
        help="Optional IAM policy JSON to add as inline policy to the role",
    )
    ap.add_argument(
        "--policy-name",
    ...
    if args.action == "full":
        ...
        if cfg.policy_name:
            print(
                f"Attaching policy '{cfg.policy_name}' to role '{cfg.role_name}' from config"
            )
    ...
    if args.action in ("full", "layer"):
        if not cfg.layers:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                for layer_def in cfg.layers:
                    ...
                    with open(layer_zip, "rb") as f:
                        layer_bytes = f.read()
                    arn = publish_layer(
                        lmb, layer_def.layer_name, layer_bytes, [cfg.runtime]
                    )
            ...
            existing = (
                ...
                else get_existing_layers(lmb, cfg.function_name)
            )
            layer_arns = (
                new_layer_arns
                if args.replace_layers
                else _merge_layers(existing, new_layer_arns)
            )
            ...
            retry_with_backoff(
                attach_layers, max_retries=5, base_delay=3.0, max_delay=15.0
            )
            print("Layer(s) attached.")
    else:
        print(
            "Skipping layer processing (use action 'layer' or 'full' to process layers)"
        )
