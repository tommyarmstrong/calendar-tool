...
def ensure_lambda_permission_for_apigw(
    ...
    source_arn = f"arn:aws:execute-api:{region}:{account_id}:{api_id}/*/{method.upper()}/{path_nolead}"
...
def process_http_gateways(
    *,
    ...
    if not routes:
        raise ValueError(
            "At least one route must be provided (via 'routes' or 'routes_file')."
        )
    ...
    for r in routes:
        ...
        if r.method not in {"GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS", "HEAD"}:
            print(
                f"WARNING: Unusual HTTP method '{r.method}'. Proceeding anyway.",
                file=sys.stderr,
            )
